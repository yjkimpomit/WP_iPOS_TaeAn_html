(function () {
                ...
                [tdNumber, tdId].forEach(el =>
                    el.addEventListener("click", () => {
                        ...
                        if (ckType === "pmt") {
                            ...
                            const panTarget = getPanTargetElement(targetElement);
							const bbox = getUserSpaceBBoxWithTranslate(panTarget);
    ...
    function renderTableUtag(dataList, idTableBody) {
        if (!dataList || dataList.length === 0) {
            console.warn("No data to render in UTag table.");
            return;
        }
        idTableBody.innerHTML = ""; // 기존 내용 초기화
        dataList.forEach((data, index) => {
            const row = document.createElement("tr");
            const tdNumber = document.createElement("td");
            tdNumber.textContent = `${index + 1}`;
            const tdKey = document.createElement("td");
            tdKey.textContent = data.key_tag || "";
            const tdUTag = document.createElement("td");
            tdUTag.textContent = data.uTagVal || "";
            row.append(tdNumber, tdUTag);
    ...
    // svg 파일에서 해당 id 값 가진 요소 추출 -> id 값 , (콤마) 기준 포함시 해당 요소 추출 
...
function findElementByMultiId(svgDoc, targetId) {
  ...
  const allWithId = svgDoc.querySelectorAll("[id]");
  const trimmedTarget = normalizeIdValue(targetId);

  for (const el of allWithId) {
    const rawId = el.getAttribute("id") || "";
    const idList = rawId
      .split(",")
      .map(s => normalizeIdValue(s))
      .filter(Boolean);

    if (idList.includes(trimmedTarget)) return el;
  }
  return null;
}
    ...
    function runSearchTag_vnet(targetId, svgDoc, panZoom, reSetStyleFn) {
        ...
        const panTarget = getPanTargetElement(item);
		const bbox = getUserSpaceBBoxWithTranslate(panTarget);
	...
	function normalizeIdValue(s) {
	  // id에 \r\n, \n 들어오는 케이스 정규화 (매칭 안정화)
	  return (s || "").replace(/[\r\n\s]+/g, "").trim();
	}
	
	function parseTranslate(transformStr) {
	  // translate(1090, 3023.656) / translate(1090 3023.656) / translate(1090) 등 대응
	  if (!transformStr) return { tx: 0, ty: 0 };
	
	  const m = transformStr.match(/translate\(\s*([-\d.]+)(?:[,\s]+([-\d.]+))?\s*\)/i);
	  if (!m) return { tx: 0, ty: 0 };
	
	  const tx = parseFloat(m[1] || "0") || 0;
	  const ty = parseFloat(m[2] || "0") || 0;
	  return { tx, ty };
	}
	
	function getAccumulatedTranslate(el, stopAtSvg = true) {
	  let tx = 0, ty = 0;
	  let cur = el;
	
	  while (cur) {
	    if (cur.nodeType !== 1) break; // ELEMENT_NODE only
	
	    const tAttr = cur.getAttribute && cur.getAttribute("transform");
	    if (tAttr) {
	      const t = parseTranslate(tAttr);
	      tx += t.tx;
	      ty += t.ty;
	    }
	    ...
	    if (stopAtSvg && cur && cur.tagName && cur.tagName.toLowerCase() === "svg") break;
	  }
	  return { tx, ty };
	}
	
	function getUserSpaceBBoxWithTranslate(el) {
	  // el.getBBox()는 로컬 bbox이므로, 조상 translate를 누적해서 user-space로 보정
	  const bbox = el.getBBox();
	  const { tx, ty } = getAccumulatedTranslate(el, true);
	...
	function getPanTargetElement(item) {
	  if (!item) return null;
	
	  // 2번 케이스: 바깥 g(transform=translate)가 실제 위치를 갖고 있음
	  const wrapper = item.closest && item.closest("g[transform]");
	  if (wrapper) return wrapper;
	
	  // 1번 케이스: text에 translate가 있을 수 있음
	  const text = item.querySelector && item.querySelector("text");
	  return text || item;
	}
    ...
    window.renderTableUtag = renderTableUtag;
