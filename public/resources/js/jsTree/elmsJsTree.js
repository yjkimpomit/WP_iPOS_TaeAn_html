document.write('<link rel="stylesheet" href="/resources/js/jsTree/css/style.css">');
document.write('<script src="/resources/js/jsTree/js/jstree.js"></script>');
	...
	$(targetId).jstree({
		...
		"contextmenu":{
			"items": function () {
				return {
					"RootCreate": {
						"label": "최상위 추가",
						"icon" : "/resources/images/icon/root_add.svg",
					...
					"Create": {
						...
						"label": "서브 등록",
						"icon" : "/resources/images/icon/sub_add.svg",
						"action": function (data) {
					...
					"Rename": {
						"separator_before" : true,
						"label": "이름 수정",
						"icon" : "/resources/images/icon/edit.svg",
						"action": function (data) {
					...
					"Delete": {
						"separator_before" : true,
						"label": "삭제",
						"icon" : "/resources/images/icon/remove.svg",
						"action": function (data) {
							var ref = $.jstree.reference(data.reference),
								sel = ref.get_selected();
							if(!sel.length) { return false; }
		...
		jsonArr.forEach(function(item){
			...
			if($("#"+item.id+" > a").find("span").length == 0){
				if(item.li_attr.isRegisted){
					setHtml = "<span style='padding-left:8px'><img src='/resources/images/icon/check.svg' style='width:20px;height:20px'></span>";
				}else{
					setHtml = "<span style='padding-left:8px'><img src='/resources/images/icon/uncheck.svg' style='width:20px;height:20px'></span>";
				}
		...
		jsonArr.forEach(function(item){
			if($("#"+item.id+" > a").find("span").length == 0){
				var setHtml = "";
				if(item.li_attr.isRegisted){
					setHtml = "<span><img src='/resources/images/icon/check.svg' style='width:20px;height:20px'></span>";
				}else{
					setHtml = "<span><img src='/resources/images/icon/uncheck.svg' style='width:20px;height:20px'></span>";
				}
				$("#"+item.id+" > a").append(setHtml);
			}
		});
